...
async def planed_voice_check(channel: discord.TextChannel, guild: discord.Guild, members, voice_channel): # Используется для проверки участников в голос. канале (не считая отработок)
    ...
    for member in members:
        if member not in checked:
            ...
            new_student_skips = student_info['skips'] + 1
            await loop.run_in_executor(None, db.update_student_skips, new_student_skips, member.id)
            await loop.run_in_executor(None, db.add_skip, member.id, str(datetime.datetime.now().strftime('%d.%m.%Y')))
...
@bot.command() # Функция регистрации. Проверяет все данные и отправляет обратную связь. Для админов
async def reg(ctx, user: discord.Member = None, name=None, last_name=None, role_id=None, *, devman_url=None):
    author = ctx.message.author
    ...
    if ADMIN_ROLE_ID in author_roles:
        loop = asyncio.get_running_loop()
        if name is None or last_name is None or role_id is None or devman_url is None or user is None:
        ...
        # user = guild.get_member(author.id)
        # await user.add_roles(role)
        await loop.run_in_executor(None, db.add_student, (user.id, f'{name} {last_name}', role.id, devman_url, 0, ''))
        await ctx.reply(f'Теперь пользователь {user.name} назначен в группу {role.name}.')
